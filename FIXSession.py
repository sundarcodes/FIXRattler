	        ...
	        while(1):
			 ...
			 if (len(data)<8192):
		            break;	
                            ...
                            self.loggerObj.debug("Receiving lot of messages...");
	                 begin=time.time();
		      else:
		         time.sleep(0.1);
	           except Exception:
        ...
        def recvFIXMsg(self):
            try:
               ...
               #Get the body length
                        ...
                        inMsg=bytesbuf[bytesParsed:msgTypeIndex+bodyLen+7];
                        self.pendingMsg=bytesbuf[msgTypeIndex+bodyLen+7:];
			#self.loggerObj.debug("Body Buf="+bytesbuf+",Body Len Index:"+str(bodyLen)+",MsgType Index:"+str(msgTypeIndex)+",inMsg="+inMsg+",pendingMsg="+self.pendingMsg);
			if self.doLogging:
                          #inMsg1=bytesbuf[bytesParsed:bytesParsed+11+(msgTypeIndex-bodyLenIndex)+bodyLen+8].replace(self.fixmsgseparator,'@');
                          inMsg1=inMsg.replace(self.fixmsgseparator,'@');
                          if (inMsg1 != ''):
                                self.lockObj.acquire(True);
                                timestamp=datetime.datetime.now().strftime("%H:%M:%S.%f");
			...
			#self.loggerObj.debug("MsgLen:"+str(msgLen)+",bytesParsed:"+str(bytesParsed)+",bytesLeftTobeParsed:"+str(bytesLeftTobeParsed));
                        #Parse the message
